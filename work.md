...
{% for entry in collections.work %}{% if entry.data.date %}{{ entry.data.date | postDate }}{% endif %} <a href="{{ entry.url }}">{{ entry.data.title  | truncate: 30}}</a>{% unless forloop.last %}\{% endunless %}
{% endfor %}